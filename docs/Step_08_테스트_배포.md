...
- 자동차 공장에서 생산 라인을 거친 차량은 출고 전에 여러 번의 점검을 받습니다. 나사 하나라도 풀려 있으면 도로에 나오기 전에 잡아내야 합니다.
- 단위 테스트는 부품 검사처럼 작은 단위의 기능을 빠르게 확인하고, 통합 테스트는 완성차를 시험 주행하는 절차와 같습니다.
- 자동화된 테스트는 "시동을 걸고, 브레이크를 밟고, 경고등을 확인한다" 같은 점검표를 버튼 한 번으로 수행해 주기 때문에, 사람보다 더 꾸준하고 정확하게 품질을 보장합니다.
...
- 같은 레시피라도 집마다 불 세기나 조리도구가 달라 맛이 달라질 수 있습니다.
- 반면 밀키트는 재료와 양념이 정확히 계량되어 있어 어디서 조리하든 동일한 맛이 나도록 도와줍니다. Docker 역시 실행 환경을 표준화해 "어디서 돌리든 같은 결과"를 보장합니다.
