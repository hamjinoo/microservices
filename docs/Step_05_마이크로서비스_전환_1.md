...
### 실생활 비유: 대형마트와 전문 상점 거리

- 모놀리식 구조는 한 건물 안에 식품, 의류, 약국이 모두 들어 있는 대형마트와 같습니다. 편하지만 한 코너만 고장 나도 전체 문을 닫아야 합니다.
- 마이크로서비스 구조는 같은 거리에 베이커리, 약국, 세탁소 등 전문 점포가 각각 운영되는 모습과 비슷합니다. 가게 하나가 문을 닫아도 다른 가게는 그대로 영업할 수 있고, 인기 있는 가게만 별도로 확장하기도 쉽습니다.
...
**실생활 비유**: 대형 쇼핑몰 안내데스크
- 매장에서 "약국이 어디에 있나요?"라고 물으면 안내데스크가 최신 매장 위치를 알려 줍니다.
- 새로운 매장이 입점하거나 이동해도 안내데스크 정보만 갱신하면 방문객이 길을 잃지 않습니다. Eureka Server가 서비스 이름을 실제 위치(IP)와 연결해 주는 역할을 합니다.
